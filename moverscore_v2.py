...
def collate_idf(
    arr, tokenize, numericalize, idf_dict, pad="[PAD]", device=torch.device("cpu")
):
...
def get_bert_embedding(
    all_sens, model, tokenizer, idf_dict, batch_size=-1, device=torch.device("cpu")
):
    ...
    with torch.no_grad():
        for i in range(0, len(all_sens), batch_size):
            outputs = model(
                padded_sens[i : i + batch_size], attention_mask=mask[i : i + batch_size]
            )
